...
DsJwtAuth.checkToken = async function _checkToken() {
  ...
  const now = moment();
  const needToken = noToken || tokenExpirationTimestamp.add(
    tokenReplaceMin, 'm',
  ).isBefore(now);
...
DsJwtAuth.getUserInfo = async function _getUserInfo() {
  ...
  const dsApi = new docusign.ApiClient();
  const { targetAccountId } = dsConfig;
 ...
 * @function
 */
DsJwtAuth.clearToken = function clearToken() { // "logout" function
